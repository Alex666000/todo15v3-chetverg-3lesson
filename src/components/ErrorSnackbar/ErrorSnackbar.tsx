...
// classes удалим
    // Так как мы контролируем ошибку в глобальном стейте то - при нажатии на крестик не закрывается - как закрыть?
    // Надо чтобы в глобальном стейте error: 'Some error' - превратился в null ---> dispatch(setAppErrorAC(null))
}
// Дебажим Материал UI и смотрим как работатет кадое свойство например open и смотрим АPI MUI - ставим несколько брекпоинтов
// Snackbar - тот алерт который всплывашкой выскакивает
// как правило всегда стейт контролируемый делаем, а не локальным стейтомю... и видим что Snackbar контролируемый на вход передается open (видимый)
// autoHideDuration через 3 сек. вызывает колбек на onClose - это handleClose
// если изначально не видим ошибку - то меняем руками open на true и видм ее уже...прописываем условия зависимости UI от стейта(глобального или локального)
// потом ставим контроль через redux, диспатчим изменения и диспатчим изменения в санке
...
- тесты пишем
- потом делаем появление ошибки в правильный момент: Когда может упасть ошибка? Когда например отправим сильно длинный title
 а сервак в "texarea" для таски (> 100 символов).
- в идеале проверки на ошибки надо делать на фронте и на бэке - и там и там - они друг другу не доверяют см. addTaskTC

-------- Валидация ошибки должна быть на сервере (бэкенд валидацию должен делать всегда!). Надо обработать ошибку
которую сервер возвращает. Смотрим ответ с сервера если ввели очень длинное название и видим что ошибка сетевого уровня а не серверная:
...
перед тем как таски получать скажем см. fetchTasksTC:
dispatch(setStatusAC("loading")) и после окончания запроса крутилку убери dispatch(setStatusAC("succeeded")). Смотрим на UI и видим крутилка убралась

- Чтобы не нажать на кнопку удаления таски (туду) много-много раз (имитировать можем поставить slow 3G...) - мы все удаляем
а на сервере все уже удалено и сервер кидает ошибку)
...
читаем что означает "в зелёном тексте" + АПИ
...
АЛГОРИТМ:
 ...
 - сделали руками зависимость UI от стейт посмотрели что страница стала статична - это свойсво
 - пишем АС,кейс, санку - санку диспатчим
 --- МЫСЛИМ СТЕЙТОМ:
 # Чтобы изменить UI сперва нужно изменить state. Чтобы изменить state нужно создать action creator, и написать case для изменения state.
 Думаем какие свойства нужны для АС при его создании...
